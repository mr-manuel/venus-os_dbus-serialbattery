...
def main():
    ...
    if port.endswith("_Ble"):
        ...
        if len(sys.argv) <= 2:
            ...
            elif port == "RoyPow_Ble":
                # noqa: F401 --> ignore flake "imported but unused" error
                from bms.roypow_ble import RoyPow_Ble  # noqa: F401

            else:
                logger.error(">>> Unknown Bluetooth BMS type: " + port)
                logger.error("Supported Bluetooth BMS types (CASE SENSITIVE!): Jkbms_Ble, Kilovault_Ble, LiTime_Ble, LltJbd_Ble, RoyPow_Ble")
                sleep(60)
                exit_driver(None, None, 1)

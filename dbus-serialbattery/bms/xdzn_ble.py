...
class Xdzn_Ble(Battery):
    ...
    def _parse_frame_data(self, raw: bytes) -> Optional[bytes]:
        ...
        if raw[3] == 0x86:
            ...
            return None
        expected_crc = _crc16(raw[:-3])
        received_crc = struct.unpack(">H", raw[-3:-1])[0]
        if expected_crc != received_crc:
            logger.warning("XDZN_BLE: CRC mismatch (expected 0x%04X, got 0x%04X)", expected_crc, received_crc)
            return None
    ...
    async def _async_test_connection(self) -> bool:
        ...
        try:
            ...
            if analog:
                ...
                logger.info(
                    "XDZN_BLE: test_connection found %d cells, " "Vmax=%.2f Vmin=%.2f",
                    self.cell_count,
                ...
                logger.warning(
                    "XDZN_BLE: could not read analog data in test_connection, " "using default %d cells",
                    self.cell_count,
                )
    ...
    async def _async_refresh_data(self) -> bool:
        ...
        try:
            ...
            if warning:
                ...
                self.protection.high_voltage = 2 if (r1 & 0x04) else 0
                self.protection.low_voltage = 2 if (r1 & 0x08) else 0
                self.protection.high_charge_current = 2 if (r1 & 0x10) else 0
                self.protection.high_discharge_current = 2 if (r1 & 0x20) else 0
                self.protection.high_charge_temperature = 2 if (r2 & 0x01) else 0
                self.protection.high_temperature = 2 if (r2 & 0x02) else 0
                self.protection.low_charge_temperature = 2 if (r2 & 0x04) else 0

                # battery_mode: 0x01=charging, 0x02=discharging, 0x03=both, 0x00=standby
                self.charge_fet = bool(mode & 0x01)
                self.discharge_fet = bool(mode & 0x02)
